...
function connect(tree, loop){
  ...
  if(children.length === 0){ //leaf node, no children
    return [{
      'name' : par.name,
      'page' : par.generator([], par.par, par.siblings[0], par.siblings[1])
    }]
  } else {
    ...
    var pages = [{
      'name' : par.name,
      'page' : par.generator(childrenRecords, par.par, par.siblings[0], par.siblings[1])
    }];
    //concatenate the page with the pages of all the children
    for(var i = 0; i < children.length; i++){
      pages = pages.concat(connect(children[i], loop));
    }
    return pages;
  }
}
